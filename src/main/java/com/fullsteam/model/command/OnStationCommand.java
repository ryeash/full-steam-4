...
public class OnStationCommand extends UnitCommand {
    ...
    // Patrol behavior - nonagon (9-sided) waypoint system
    private List<Vector2> patrolWaypoints;
    private int currentWaypointIndex = 0;
    private static final int PATROL_SIDES = 24; // Nonagon for smoother but not perfectly circular patrol
    ...
    public OnStationCommand(Unit unit, Vector2 stationLocation, boolean isPlayerOrder) {
        ...
        this.patrolRadius = 120.0; // Circle radius for patrol
        
        // Pre-calculate nonagon waypoints around the station
    ...
    private List<Vector2> calculatePatrolWaypoints() {
        ...
        for (int i = 0; i < PATROL_SIDES; i++) {
            ...
            double x = stationLocation.x + Math.cos(angle) * patrolRadius;
            double y = stationLocation.y + Math.sin(angle) * patrolRadius;
    ...
    public boolean update(double deltaTime) {
        ...
        if (!onStation && distanceToStation < patrolRadius * 0.5) {
            onStation = true;
            log.info("Interceptor {} on station at ({}, {})", 
                    unit.getId(), (int)stationLocation.x, (int)stationLocation.y);
        }
